...
select 
	...
	on columns,
	non empty topcount(
	[Dim Tiempo].[Nombre Mes].[Nombre Mes],1
	)on rows
from [PROYECTO1_COMPRA_CUB]
where [Dim Tiempo].[Anio].[2013];
...
select 
	...
	non empty (
	{[Dim Articulo].[Color Base].[BLANCO], [Dim Articulo].[Color Base].[ROJO],
	[Dim Articulo].[Color Base].[AMARILLO], [Dim Articulo].[Color Base].[AZUL]},
	[Dim Tiempo].[Anio].[Anio]
	)on rows
from [PROYECTO1_COMPRA_CUB];
...
WITH MEMBER [Measures].[PRECIO] as
	[Measures].[#total]/[Measures].[#cantidad Articulos]
select 
non empty {
[Measures].[PRECIO] 
}on columns,
...
select 
	...
	[Dim Sub Compra].[Id Compra].[Id Compra],
	[Dim Sub Compra].[Folio].[Folio],
	[Dim Tiempo].[Nombre Mes].&[November],
	[Dim Tiempo].[Anio].[2012]
	)  on rows
from [PROYECTO1_COMPRA_CUB]
where [Dim Tiempo].[Nombre Dia].&[Thursday];
...
WITH 
	...
	MEMBER [Measures].[total] as 
			[Measures].[Recuento Fact Compra]*([Measures].[#total]) 
	MEMBER [Measures].[precio] as
		(([Measures].[#subtotal]-[Measures].[#descuento]+[Measures].[#IVA])/[Measures].[#cantidad Articulos])
select 
	{
	[Measures].[#cantidad Articulos],
	[Measures].[precio],
