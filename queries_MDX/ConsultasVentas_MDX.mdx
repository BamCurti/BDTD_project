...
select 
	...
	[Dim Articulo 1].[Color Base].&[ROJO],
	[Dim Tiempo 1].[Anio].[2014]
	)on rows
from [PROYECTO1_VENTA_VOD];
...
select 
	...
	(
	topcount(
	([Dim Articulo 1].[Color Derivado].[Color Derivado]-[Dim Articulo 1].[Color Derivado].&[N/A]),
	5,[Measures].[#num Articulos]),
	[Dim Tiempo 1].[Anio].[Anio]
	)on rows
from [PROYECTO1_VENTA_VOD]
where [Dim Articulo 1].[Tipo].&[ESMALTES];
